...
class PredictionPipelineConfig:
    model_file_path: str = os.path.join(artifact_folder, "model.pkl")
    preprocessor_path: str = os.path.join(artifact_folder, "preprocessor.pkl")
    prediction_output_path: str = os.path.join("predictions", "prediction_file.csv")
...
class PredictionPipeline:
    ...
    def save_input_file(self):
        try:
            ...
            input_csv_path = os.path.join("prediction_artifacts", "test.csv")
            input_csv_file.save(input_csv_path)
    ...
    def predict(self, features: pd.DataFrame):
     try:
        ...
        # 1. Drop the target column if it exists
        if TARGET_COLUMN in features.columns:
            features = features.drop(columns=[TARGET_COLUMN], axis=1)

        # 2. DROP THE "prediction" COLUMN IF IT EXISTS
        if "prediction" in features.columns:
            features = features.drop(columns=["prediction"], axis=1)

        # 3. Drop any other non-numeric columns (like Wafer IDs)
        non_numeric_cols = features.select_dtypes(exclude=[np.number]).columns.tolist()
        if non_numeric_cols:
            features = features.drop(columns=non_numeric_cols, axis=1)

        # Now the features will match exactly what the preprocessor expects
        transformed_x = preprocessor.transform(features)
        preds = model.predict(transformed_x)
    ...
    def get_predicted_dataframe(self, input_csv_path: str):
        try:
            ...
            os.makedirs(os.path.dirname(self.prediction_pipeline_config.prediction_output_path), exist_ok=True)
            output_df.to_csv(self.prediction_pipeline_config.prediction_output_path, index=False)

            return self.prediction_pipeline_config.prediction_output_path
    ...
    def run_pipeline(self):
        try:
            ...
            prediction_file_path = self.get_predicted_dataframe(input_csv_path)
            return self.prediction_pipeline_config
